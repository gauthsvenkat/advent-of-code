...
fn parse_diskmap(diskmap: &str) -> HashMap<i32, (u32, u32)> {
    let mut is_freespace: bool = false;

    let mut diskmap_record = HashMap::new();

    let mut file_id: i32 = 0;
    let mut free_id: i32 = -1;
    let mut diskmap_idx: u32 = 0;

    for c in diskmap.chars() {
        let size: u32 = c.to_digit(10).unwrap();

        let id = if is_freespace { free_id } else { file_id };

        diskmap_record.insert(id, (diskmap_idx, size));
...
fn fragment_blocks(record: HashMap<i32, (u32, u32)>) -> Vec<u32> {
...
fn p1(input: &str) -> usize {
    ...
    let record = parse_diskmap(&diskmap);
    dbg!(record);
    0
}
